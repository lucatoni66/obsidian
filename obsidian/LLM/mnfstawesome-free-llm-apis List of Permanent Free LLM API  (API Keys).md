---
title: "mnfst/awesome-free-llm-apis: List of Permanent Free LLM API  (API Keys)"
source: "https://github.com/mnfst/awesome-free-llm-apis"
author:
  - "[[mnfst]]"
published:
created: 2026-04-23
description: "List of Permanent Free LLM API  (API Keys). Contribute to mnfst/awesome-free-llm-apis development by creating an account on GitHub."
tags:
  - "clippings llm api"
---
LLM APIs with permanent free tiers for text inference.

<sub>All endpoints are OpenAI SDK-compatible unless noted. Each link points to the provider's API key page.</sub>
...
Free tier with qualitative usage limits. 400+ models from Ollama library. Not OpenAI SDK-compatible; uses [Ollama API](https://docs.ollama.com/cloud). [^5]
...
35+ free models (marked with `:free` suffix). OpenAI SDK-compatible. [^6]
...
Know a free tier that's missing? [Open a PR](https://github.com/mnfst/awesome-free-llm-apis/blob/main/contributing.md). Include the provider, endpoint, rate limits (link to their docs), and a few notable models. Trial credits and time-limited promos don't count.
...
[^3]: Kilo Code free model list may change over time. nvidia/nemotron-3-super-120b-a12b:free is for trial use only — prompts are logged by NVIDIA. Auto-router `kilo-auto/free` routes to minimax/minimax-m2.5:free (80%) and stepfun/step-3.5-flash:free (20%).
...
[^5]: Ollama Cloud measures usage by GPU time, not tokens or requests. Free tier described as "light usage" with session limits resetting every 5 hours and weekly limits every 7 days. Pro (50x more) and Max (250x more) plans available. Not OpenAI SDK-compatible; uses the Ollama API.

[^6]: Free models default to 200 RPD. A one-time purchase of $10+ in credits unlocks 1,000 RPD for free models. OpenRouter also offers a [Free Models Router](https://openrouter.ai/docs/guides/routing/routers/free-models-router) (`openrouter/free`) and [model fallbacks](https://openrouter.ai/docs/guides/routing/model-fallbacks) for chaining models in priority order.